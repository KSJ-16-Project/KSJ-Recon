...
async def run_auth_layer(
    browser: Browser,
    pages: list[Any],
    config: AuthConfig,
) -> AuthResult:
    """Try optional login from first-crawl pages, returning a non-fatal result."""
...
def _normalise_pages(pages: list[Any]) -> list[dict]:
    ...
    for page in pages:
        ...
        if forms is None:
            html = data.get("rendered_html", "") or data.get("html", "")
            forms = extract_forms(html, url) if html else []
        out.append({
            **data,
            "url": url,
