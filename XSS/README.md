...
스캔 중 `Ctrl+C`를 누르면 진행된 결과를 `results/xss_result_partial.json`에 저장합니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `base_url` | string | 스캔 대상의 루트 URL. 없으면 스캐너가 즉시 종료됨 |
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `evidence_dir` | string | `XSS/evidence/` | 브라우저 검증 스크린샷 저장 경로 |
...
| `dom_stored_xss` | bool | `true` | Playwright 기반 DOM Stored XSS 검증 |
...
| `safe_to_submit` | bool | `false` | `true`면 Stored XSS용 실제 POST 제출 허용 |
...
| **브라우저 검증 (alert 확인)** | `options.browser_verify: true` + Reflected 후보 존재 |
| **Stored XSS (HTTP)** | `urls[]`에 `method: "POST"`, `safe_to_submit: true`, `params` 설정 + `check_urls` 권장 |
...
> `safe_to_submit: true` 없이는 POST 폼에 실제 데이터를 전송하지 않습니다. 운영 데이터가 오염되는 것을 방지하기 위한 안전장치입니다.
...
- Playwright 기반 alert 브라우저 검증 (스크린샷 포함)
- WAF 감지 및 우회 페이로드 시도
- CSRF 토큰 자동 추출 및 주입
...
- `safe_to_submit: true` 없이는 POST 폼에 실제 데이터 전송 없음
