...
crawler engine이 public 크롤 완료 후 get_session()으로 세션을 요청한다.
"""
...
async def get_session(browser: Browser) -> AuthResult:
    """
    저장된 자격증명으로 로그인을 수행하고 AuthResult를 반환한다.
    crawler engine이 public 크롤 완료 후 호출한다.

    Returns:
        성공: AuthResult(success=True, cookies=[...])  — cookies가 2차 크롤에 사용됨
        실패: AuthResult(success=False, error="...")
    """
...
async def _detect_selectors(browser: Browser, url: str) -> FormSelectors | None:
    """
